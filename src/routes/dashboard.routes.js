...
// --- Debug: test Mindbody search directly (no auth middleware — uses inline token check) ---
router.get('/debug/mindbody-search', async (req, res) => {
  const { q, type = 'client', token } = req.query;
  if (!token || token !== process.env.DASHBOARD_API_TOKEN) {
    return res.status(401).json({ error: 'Pass ?token=YOUR_DASHBOARD_API_TOKEN' });
  }
  try {
    if (!q) return res.status(400).json({ error: 'Pass ?q=searchterm' });
    ...
    const mbToken = tokenRes.data.AccessToken;
    const headers = { authorization: mbToken };
...
// --- Debug: test Mindbody search directly ---
    if (!q) return res.status(400).json({ error: 'Pass ?q=searchterm' });
    ...
    // Get a fresh token by calling the service
    const mb = require('../services/mindbody.service');
    // Use the service's getClientByPhone with a dummy call to warm up the token,
    // then call the raw API ourselves
    const api = axios.create({
      baseURL: BASE_URL,
      headers: {
        'Content-Type': 'application/json',
